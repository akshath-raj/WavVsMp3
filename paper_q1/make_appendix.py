...
def write_splits_appendix() -> None:
    ...
    w("")
    w(r"The tables below list every internal node of the depth-3 entropy tree "
      r"of Section~\ref{sec:trees}. The tree was fitted independently on each "
      r"condition, using the same actors, the same items and the same "
      r"hyper-parameters. The \emph{Path} column reads from the root, and "
      r"\ttf{L} denotes the branch taken when the parent's test is satisfied. "
      r"Entropies are given in bits. The value $n$ is the raw sample count, and "
      r"the information gain is computed on the class-weighted counts "
      r"(Eq.~\ref{eq:ig}).")
    w("")
