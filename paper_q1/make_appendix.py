...
DESCRIPTORS: list[tuple[str, str, str, str, str]] = [
    ...
    ("spectral_centroid", "Spectral centroid",
     "First moment of the power spectrum; the frequency about which spectral energy is balanced. Correlates with perceived brightness.",
     STATS8, "spectral"),
    ("spectral_bandwidth", "Spectral bandwidth",
     "Power-weighted second moment about the centroid; how widely energy is spread in frequency.",
     STATS8, "spectral"),
    ...
    ("spectral_flatness", "Spectral flatness",
     "Geometric mean of the magnitude spectrum divided by its arithmetic mean. Approaches 1 for noise-like frames and 0 for tonal ones; collapses when an encoder zeroes masked bins.",
     STATS8, "spectral"),
    ("spectral_zcr", "Zero-crossing rate",
     "Fraction of adjacent sample pairs with a sign change; a cheap voicing and frication proxy.",
     STATS8, "spectral"),
    ("spectral_rms", "Frame RMS energy",
     "Root-mean-square magnitude per frame; the loudness contour.",
     STATS8, "spectral"),
    ("spectral_flux", "Spectral flux",
     "Euclidean norm of the frame-to-frame difference of the magnitude spectrum; how fast the spectrum is changing.",
     STATS8, "spectral"),
    ("spectral_entropy", "Spectral entropy",
     "Shannon entropy of the power spectrum normalised to its own bin count (Eq.~\\ref{eq:specent}); a scale-free measure of how evenly energy is distributed across frequency.",
     STATS8, "spectral"),
    ("spectral_slope", "Spectral slope",
     "Least-squares slope of magnitude regressed on frequency, per frame; spectral tilt.",
     STATS8, "spectral"),
    ...
    ("mfcc_d2_", "$\\Delta\\Delta$MFCC (20)",
     ...
     STATS2, "mfcc"),
    ("mfcc_", "MFCC (20)",
     "Discrete cosine transform of the log mel-filterbank energies; the standard SER front end. MFCC-0 tracks overall log energy, so its derivatives measure how fast loudness changes.",
     STATS8, "mfcc"),
    # ---- chroma -------------------------------------------------------------
    ...
    ("prosody_f0_slope_abs_mean", "$F_0$ slope (mean $|\\cdot|$)",
     "Mean absolute first difference of the voiced $F_0$ contour, per second; pitch movement rate.",
     "scalar", "prosody"),
    ...
    ("prosody_hnr_", "Harmonics-to-noise ratio",
     "Ratio of periodic to aperiodic energy in decibels; a hoarseness and breathiness correlate.",
     STATS5, "prosody"),
    ...
    ("prosody_f", "Formants $F_1$--$F_5$",
     "Burg-estimator formant frequencies; the vocal-tract filter. $F_2$ movement is the strongest single correlate of disgust in this corpus.",
    ...
    ("prosody_pause_fraction", "Pause fraction",
     "Complement of the voiced fraction; definitionally $1 -$ voiced fraction ($r = -1.00$).",
     "scalar", "prosody"),
    ...
    ("prosody_voiced_rate_per_s", "Voiced-segment rate",
     "Voiced segments per second; a speech-rate proxy.",
     "scalar", "prosody"),
    ("prosody_mean_voiced_seg_s", "Mean voiced-segment duration",
     "Mean length in seconds of a voiced run.",
     "scalar", "prosody"),
    ("prosody_cpps", "CPPS",
     "Smoothed cepstral peak prominence; the standard acoustic correlate of breathy phonation. Drives both sadness and neutrality in every attribution method applied here.",
     "scalar", "prosody"),
    # ---- global -------------------------------------------------------------
...
def main() -> None:
    ...
    w(r"Analysis frames are \SI{512}{point} FFT, \SI{400}{sample} window, "
      r"\SI{160}{sample} hop at \SI{16}{\kilo\hertz} --- \SI{25}{\milli\second} "
...
def write_splits_appendix() -> None:
    """Every internal node of the depth-3 entropy tree, for all four conditions."""
    import json

    from xai_ser.paths import OUTPUTS

    pc = sorted((OUTPUTS / "per_condition").glob("2*"))[-1]
    summary = json.loads((pc / "summary.json").read_text())
    splits = summary["top_splits"]
    ...
    w("")
    w(r"Every internal node of the depth-3 entropy tree of "
      r"Section~\ref{sec:trees}, fitted independently on each condition with the "
      r"same actors, the same items and the same hyper-parameters. \emph{Path} "
      r"reads from the root: \ttf{L} is the branch taken when the parent's test "
      r"is satisfied. Entropies are in bits; $n$ is the raw sample count and the "
      r"information gain is computed on the class-weighted counts "
      r"(Eq.~\ref{eq:ig}).")
    w("")

    for cond in ("ref", "mp3_64", "mp4_aac64", "roundtrip_wav"):
        w(r"\begin{table}[!ht]")
        w(rf"\caption{{Depth-3 tree fitted on \textbf{{{COND_NAME[cond]}}}.}}")
        ...
        w(r"\midrule")
        for n in splits[cond]:
            path = n["path"] or r"\textit{root}"
            w(rf"\texttt{{{path}}} & \texttt{{{esc(n['feature'])}}} & "
              rf"{n['threshold']:.4f} & {n['entropy_bits']:.4f} & "
              rf"{n['information_gain_bits']:.4f} & {n['n_samples']}\\")
        w(r"\bottomrule")
        w(r"\end{tabular}")
        w(r"\end{table}")
        w("")
    ...
    dest.write_text("\n".join(out) + "\n")
    print(f"wrote {dest}  (4 conditions x {len(splits['ref'])} internal nodes)")
