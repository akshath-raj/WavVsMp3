# Does the Container Change the Evidence? A Black-Box XAI Protocol for Testing Audio Format Robustness in a Multimodal Large Language Model
...
Every deployed audio pipeline transcodes, yet evaluations of large audio-language models (LALMs)
do not report the container in which audio was submitted. This report specifies and stress-tests a
protocol for determining whether lossy container delivery (MP4/AAC at 64 kbps) changes not only a
multimodal LLM's accuracy on speech emotion recognition but *which regions of the signal its
decision depends on*. The design's distinguishing feature is a contrast that holds the decoded
...
questions; those await gate clearance described in §3.7. Presenting the design and its
feasibility constraints before data collection is deliberate: the study predicts, from published
evidence, that its own headline accuracy contrast will be flat, and a study in that position must
fix its interpretation rules in advance or it will find whatever it needs to find.
...
vector per item per condition. R was raised from 3 to 7 after adversarial review, because at R = 3
the measure takes only four values and ties would dominate the rank statistics, biasing SQ3 toward
a null irrespective of the truth.
...
Neither signal is sufficient alone: n = 8 is far too small for inference, and Chen et al. is one
benchmark whose model coverage may not include the model under study, standing against Zhang et
al. (2026), who benchmark speech LLMs for SER at considerable scale. But two independent weak
signals pointing the same direction justify treating gate G0 as a live risk rather than a
formality — which is what the gate order now encodes.
...
A passed equivalence test on the container contrast would be a genuinely useful negative result. It
would license format-agnostic submission for this model at this operating point and would justify
the field's implicit assumption with evidence rather than plausibility.
...
This report specifies a protocol for a question that current practice has not asked — whether the
container an audio file arrives in changes what a multimodal LLM's decision rests on — and reports
the preparatory work that reshaped it. The literature places the study between a mature
codec-robustness tradition, a young LALM-evaluation literature that omits format entirely, and an
XAI tradition establishing that explanations can shift while predictions hold. Three adversarial
checkpoints and a data audit produced changes that a post-hoc review would not have caught in time:
an unreachable equivalence bound, an undefined attribution measure under degenerate responses, and
a pilot that failed on quota rather than on method.
...
2. **Run gate G0** — reference accuracy on ≥ 20 items × 3 repeats — before anything else, and
   decide in advance whether the 40% threshold is a deliberate human-parity benchmark
   (Cao et al., 2014) or should be a lower headroom criterion.
...
| 4 | `phase4_composition/report_draft.md` (this document) |
| 5 | `phase5_review/` — editorial, ethics, DA Checkpoint 3 |
| 6 | `phase6_final/` — final report and handoff pack |
